...
const AssetAllocation = () => {
  ...
  const filteredAllocations = allocations.filter(allocation => {
    const assetName = allocation.asset?.name || '';
    const userName = allocation.user?.name || allocation.user?.email || '';
    const userDepartment = allocation.user?.department || '';
  ...
  const isAllocationOverdue = (allocation) => {
    ...
    const returnDate = new Date(allocation.expected_return_date);
  ...
  const handleNewAllocation = async () => {
    if (!newAllocation.assetId || !newAllocation.userId) {
      toast({
    ...
    try {
      const allocationData = {
        ...
        user_id: parseInt(newAllocation.userId),
        ...
        <Dialog open={isDialogOpen} onOpenChange={setIsDialogOpen}>
          ...
          <DialogContent>
            <DialogHeader>
              ...
              <DialogDescription>
                Assign an asset to a user for use.
              </DialogDescription>
            </DialogHeader>
            <div className="space-y-4">
              ...
              <div>
                ...
                <Select
                  ...
                  onValueChange={(value) => setNewAllocation({ ...newAllocation, userId: value })}
                >
                  <SelectTrigger>
                  ...
                  <TableRow key={allocation.id}>
                    <TableCell>
                      <div className="flex items-center space-x-2">
                        ...
                        <div>
                          <div className="font-medium">{allocation.asset?.name || 'Unknown Asset'}</div>
                          <div className="text-sm text-muted-foreground">
                            {allocation.asset?.brand && `${allocation.asset.brand} `}
                            {allocation.asset?.model_number && `- ${allocation.asset.model_number}`}
                          </div>
                        </div>
                      </div>
                    </TableCell>
                    <TableCell>
                      <div className="flex items-center space-x-3">
                        <Avatar className="h-8 w-8">
                          <AvatarFallback>
                            {(allocation.user?.name || allocation.user?.email || 'U').substring(0, 2).toUpperCase()}
                          </AvatarFallback>
                        </Avatar>
                        <div>
                          <div className="font-medium">{allocation.user?.name || 'Unknown User'}</div>
                          <div className="text-sm text-muted-foreground">
                    ...
                    <TableCell>{allocation.user?.role || 'Unknown'}</TableCell>
                    <TableCell>
